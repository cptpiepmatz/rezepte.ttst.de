...
async fn main() {
    ...
    let recipe_name = Arc::new(
        recipe_file_path
            .file_name()
            .unwrap_or_else(|| panic!("{} is not a file path", recipe_file_path.display()))
            .to_string_lossy()
            .replace(".rezept.txt", ""),
    );
    let recipe = Arc::new(Mutex::new(
        fs::read_to_string(&recipe_file_path).expect("could not read recipe file path"),
    ));
    ...
    let mut watcher = notify::recommended_watcher(move |_| {
        ...
        *recipe = fs::read_to_string(recipe_file_path_watch.deref())
            .expect("could not read recipe file path");
    })
    .expect("could not create watcher");
    watcher
        .watch(recipe_file_path.deref(), RecursiveMode::NonRecursive)
        .expect("could not watch recipe file path");
    ...
    let name_route = warp::path("name").map(move || recipe_name_clone.deref().clone());

    // Route for "/recipe"
    let recipe_route = warp::path("recipe").map(move || {
        let recipe = recipe.lock().unwrap();
        recipe.clone()
    });

    let images = warp::path("_REZEPTE_")
        .and(warp::path("img"))
        .and(warp::path::tail())
        .map(move |tail: Tail| {
    ...
    let static_files = warp::path::full().map(|path: FullPath| {
        ...
        if let Some(file) = WEB_DIR.get_entry(&filename).and_then(|e| e.as_file()) {
            ...
            match PathBuf::from(filename)
                .extension()
                .expect("only serve file requests")
                .to_string_lossy()
                .as_ref()
            {
